...
RUN apk add --no-cache \
      git curl jq sqlite tar gzip \
      coreutils findutils ca-certificates && \
    mkdir -p /toolbox && \
    for cmd in git curl jq sqlite3 split sha256sum \
               stat du sort tail awk xargs find \
               wc cut tr gzip tar cat date sleep \
               mkdir rm ls grep sed head chmod \
               touch cp mv basename dirname; do \
      [ -f "$(which $cmd 2>/dev/null)" ] && \
        cp "$(which $cmd)" /toolbox/ || true; \
    done && \
    ls -la /toolbox/
